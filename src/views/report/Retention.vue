<!-- Report Retention Value tab -->
<template>
  <div>
    <a-form layout="inline" :form="form" >
      <om-report-date-picker :value="dateRange" @ok="handleApply" :maxDay="30"/>
    ...
    <div style="background-color: white; margin-top: 8px; padding: 16px">
      <span style="font-size: 16px;color: #333333;font-weight:500;margin-right: 4px;">Retention by</span>
...
import { reportRetention, reportRetentionChart } from '@/api/report'
...
export default {
  ...
  methods: {
    ...
    buildBaseParams () {
      ...
      this.$emit('change', { pubAppId: ps.s_publisher_app_id || [], regions: this.regions, dateRange: ps.dateRange })
      delete ps.dateRange
      return ps
    },
    reloadChart () {
            ...
            if (!res.code) {
              ...
              if (res.data && res.data.length) {
                ...
                for (let i = 0; i < res.data.length; i++) {
                  ...
                  const fieldsSize = item.base_date === 'Mean' ? (Object.keys(item).length - 1) / 2 : (Object.keys(item).length - 9) / 2
                  for (let n = 1; n < fieldsSize; n++) {
                    const _new = {}
                    if (!item.d) {
                      _new['base_date'] = 'Mean'
                    } else {
                    ...
                    _new['day'] = n
                    _new['value'] = item['day' + n + '_u_c'] ? (item['day' + n + '_r_c'] || 0) / item['day' + n + '_u_c'] : 0
                    newList.push(_new)
                  }
                }
              }
            ...
            if (!res.code) {
              if (res.data && res.data.length) {
                ...
                res.data.map((item, index) => {
                  ...
                  for (let n = 1; n < fieldsSize; n++) {
                    item['day' + n] = item['day' + n + '_u_c'] ? (item['day' + n + '_r_c'] || 0) / item['day' + n + '_u_c'] : 0
                  }
