<template>
  <a-form-item>
    <a-select
      ...
      v-decorator="[name, {initialValue: initValue, rules: [{ required: true, message: $msg('account.adn_empty') }]}]"
      @change="handleChange">
    ...
    <a-select
      ...
      :disabled="disabled"
      optionLabelProp="title"
      v-decorator="[name, {rules: [{ required: true, message: $msg('account.adn_empty') }]}]"
      @change="handleChange">
      <a-select-option v-for="adn in optionList" :key="adn.id" :title="adn.className">
        <div class="selected-app-small">
          <img style="height:24px;" :src="'/logo/'+adn.className + '.svg'">
